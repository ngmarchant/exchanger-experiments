...
# Run for GenCoupon
expt_name <- paste0("cora_ours_blinkdist_coupon_", gsub("[ :]", "_", date()))
run_ours(expt_name, model, true_membership, n_samples = 9000, burnin_interval = 50000)

# Run for PY
expt_name <- paste0("cora_ours_blinkdist_py_", gsub("[ :]", "_", date()))
...
# Run for Ewens
expt_name <- paste0("cora_ours_blinkdist_ewens_", gsub("[ :]", "_", date()))
model@clust_prior <- EwensRP(GammaRV(1, 1/100))
model@clust_params <- EwensRP(mean(GammaRV(1, 1/100)))
run_ours(expt_name, model, true_membership, n_samples = 9000, burnin_interval = 50000)

# Run for Blink Coupon
expt_name <- paste0("cora_ours_blinkdist_blinkcoupon_", gsub("[ :]", "_", date()))
